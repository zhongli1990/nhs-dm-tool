...
## 4. Full lifecycle data pipeline deployment
...
3. Connector types API returns list including `csv`, `cache_iris_emulator`, `postgres_emulator`.

## 5.4 UI control-plane readiness checks
...
2. `/connectors`:
   - connector config and dynamic discovery works
3. `/schemas`:
   - source/target tabs, table filter, field drill-down work
4. `/mappings`:
...
## 6. Production-like operational controls

## 6.1 Release gate policy
...
4. Capture operator + timestamp in release pack.

## 6.3 Data safety controls
...
## 7. Manual E2E validation protocol

1. Open UI: `http://127.0.0.1:3133`.
2. Validate schemas page table counts match catalogs.
3. Validate mappings page has mapping classes and filter behavior.
4. Trigger run from `/runs` or POST `/api/runs/execute`.
5. Verify generated report status chain:
   - contract migration PASS
   - enterprise quality PASS
   - release gate PASS (selected profile)
6. Review rejects/issues endpoints:
...
2. Backend not starting:
   - run `run_backend.ps1` directly and inspect startup traceback.
3. Frontend cannot call backend:
   - verify `NEXT_PUBLIC_DM_API_BASE` points to `http://localhost:8099`.
4. Lifecycle fails:
   - inspect `reports/product_lifecycle_run.json` step return codes.
5. Gate fails:
   - inspect `reports/release_gate_report.json` and relevant issue CSV.

## 9. Hardening backlog for true production cutover
